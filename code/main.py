...
def funcRef(x, C=1, t=0):
    ...
    func_E = [8.928,\
             ...
             1.428]
    return [
        np.piecewise(x, conds, func_ρ),
        np.piecewise(x, conds, func_E),
        np.piecewise(x, conds, func_m)
    ]
...
def U2A(U):
    ...
    A[:, 1, 0] = (2 * Hx * U[:, 0] *
                  (U[:, 5] * U[:, 3] + U[:, 6] * U[:, 4]) + U[:, 2] *
                  (U[:, 0] * (-ga * U[:, 1] + (ga - 2) * U[:, 5]**2 +
                              (ga - 2) * U[:, 6]**2) + 2 *
                   (ga - 1) * U[:, 3]**2 + 2 * (ga - 1) * U[:, 4]**2) + 2 *
                  (ga - 1) * U[:, 2]**3) / U[:, 0]**3
    A[:, 1, 1] = (ga * U[:, 2]) / U[:, 0]
    A[:, 1,
      2] = -((-ga * U[:, 1] * U[:, 0] + (ga - 2) * U[:, 5]**2 * U[:, 0] +
              (ga - 2) * U[:, 6]**2 * U[:, 0] + 3 * (ga - 1) * U[:, 2]**2 +
              (ga - 1) * U[:, 3]**2 + (ga - 1) * U[:, 4]**2) / U[:, 0]**2)
    A[:, 1, 3] = -((2 * (Hx * U[:, 5] * U[:, 0] +
                         (ga - 1) * U[:, 2] * U[:, 3])) / U[:, 0]**2)
    A[:, 1, 4] = -((2 * (Hx * U[:, 6] * U[:, 0] +
                         (ga - 1) * U[:, 2] * U[:, 4])) / U[:, 0]**2)
    A[:, 1,
      5] = -((2 * (Hx * U[:, 3] + (ga - 2) * U[:, 5] * U[:, 2])) / U[:, 0])
    A[:, 1,
      6] = -((2 * (Hx * U[:, 4] + (ga - 2) * U[:, 6] * U[:, 2])) / U[:, 0])
    A[:, 2, 0] = ((ga - 3) * U[:, 2]**2 + (ga - 1) *
                  (U[:, 3]**2 + U[:, 4]**2)) / (2 * U[:, 0]**2)
    A[:, 2, 1] = (ga - 1) / 2
    A[:, 2, 2] = -(((ga - 3) * U[:, 2]) / U[:, 0])
...
def shift1(X):
    return np.pad(
        np.roll(X, 1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
...
def Lax_u_n(u, C=0.5, t=100):
    ...
    f = U2F(u)
    a = U2A(u)
    tmp_u = np.expand_dims(np.pad(u, ((1, ), (0, )), 'edge'),
                           [0, -1]).repeat(2, axis=0)
    ...
    for n in range(2):
        ...
        tmp_u[nex, :, :, :] = tmp_u[cur, :, :, :] - 0.5 * C * shift1(
            ddF) + 0.5 * C * C * shift1(Lax_sub)
        ...
        tmp_u = np.pad(tmp_u[:, 1:-1, :, :], ((0, ), (1, ), (0, ), (0, )),
                       'edge')
...
def Upwind_u(u, C=0.5, t=100):
    ...
    for n in range(t):
        ...
        Rl = np.pad(
            np.roll(R, 1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
        Rr = np.pad(
            np.roll(R, -1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
        Ll = np.pad(
            np.roll(L, 1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
        Lr = np.pad(
            np.roll(L, -1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
        dia_l = np.pad(
            np.roll(dia_, 1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )),
            'edge')
        dia_r = np.pad(
            np.roll(dia_, -1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )),
            'edge')
        ...
        up = c_u - np.pad(
            np.roll(c_u, 1, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
        um = np.pad(
            np.roll(c_u, -1, axis=0)[1:-1, :, :],
            ((1, ), (0, ), (0, )), 'edge') - c_u
        #tmp_u[nex, :, :, :] =  c_u \
...
def shift(u, i=1):
    if i == 0:
        ...
        return np.pad(
            np.roll(u, i, axis=0)[1:-1, :, :], ((1, ), (0, ), (0, )), 'edge')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-y",
                        "--ylim",
                        default="1.5,1.5,1.6,1.7,1.8,1.9,1.9",
                        type=str,
                        help="intput format")
